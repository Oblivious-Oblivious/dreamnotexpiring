  ...
  onMount(() => {
    ...
    const shuffle_distort_text = (windex) => {
      ...
      const check_length = (x) =>
        Array.from(document.querySelectorAll(x)).length > 0;
      ...
      const change_letter = (letter) => {
        if (letter.textContent != " ") {
          ...
          const new_character = work_data[windex].letters.substr(
            Math.random() * work_data[windex].letters.length,
            1
          );
      ...
      const divide_all_letters = () => {
        ...
        work_data[windex].single_letters = document.querySelectorAll(
          `.work-title-distorted-${windex}`
        );
      };
      ...
      const change_all_letters = () => {
        ...
        if ($browser_engine == "WebKit") setTimeout(change_all_letters, 800);
        else setTimeout(change_all_letters, 50);
      };
      ...
      const reset_all_letters = () => {
        ...
        random_array.forEach((el) =>
          reset_letter(
            work_data[windex].single_letters[el],
            work_data[windex].original_strings.substring(el, el + 1)
          )
        );
      };
      ...
      return (selector) => {
        if (check_length(selector)) {
          work_data[windex].target = Array.from(
          ...
          document
            .querySelector(`.work-title-container ${selector}`)
...
<div class="work-section" style="visibility: hidden;">
  ...
  <div class="work work-4">
    <a href="/work/salsa" draggable="false">
      <div
        class="work-title-container work-title-container-salsa-pomodoro-timer"
      >
        <div class="work-title work-salsa-pomodoro-timer">
  ...
  <div class="work work-13">
    <a href="/work/princeofpersia" draggable="false">
      <div class="work-title-container work-title-container-pop-on-scratch">
        <div class="work-title work-pop-on-scratch">
          POP&nbsp;on&nbsp;Scratch
        </div>
      </div>
    </a>
