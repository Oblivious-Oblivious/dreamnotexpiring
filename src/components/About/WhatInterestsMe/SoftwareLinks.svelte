<script>
    ...
    onMount(() => {
        ...
        gsap.utils.toArray(".software-link").forEach(link => {
            ...
            var fromx;
            if(`${link.classList[1].toString()}` === "software-margaret")
                fromx = { xPercent: 10 };
            else
                fromx = { xPercent: -10 };

            const reveal_text = gsap.fromTo(`.${link.classList[1].toString()} .software-overlay .software-text`, fromx, {
                paused: true,
                duration: 0.4,
                autoAlpha: 1,
                xPercent: 0,
